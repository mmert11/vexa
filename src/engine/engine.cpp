...
void vexa::engine::optimize()
{
    ...
    if (get_option(option::CONSTANT_PROPAGATION))
...
void vexa::engine::print()
{
    ...
    double expansion = asm_count ? double(ir_before_opt) / double(asm_count) : 0.0;
    ...
    std::cout << "lifted          : " << std::dec << asm_count << " insts in " << time << "\n";

    std::cout << "ir generated    : " << ir_before_opt << " ir-insts (×" << std::fixed
              << std::setprecision(2) << expansion << " expansion)\n";

    std::cout << "optimized into  : " << ir_after_opt << " ir-insts (−" << std::fixed
