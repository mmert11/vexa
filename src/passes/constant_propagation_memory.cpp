    ...
    value_simplifier &simplify_once)
{
    ...
    for (auto &BB : *function) {
        ...
        block.predecessors.assign(llvm::pred_begin(&BB), llvm::pred_end(&BB));
    }
    ...
    std::unordered_set<memory_address, memory_address_hash> concretely_written_bytes;
            ...
            if (auto *store = llvm::dyn_cast<llvm::StoreInst>(&I)) {
                ...
                for (uint64_t offset = 0; offset < access->size; ++offset) {
                    ...
                    if (byte_offset < access->address.offset)
                        break;
                    concretely_written_bytes.insert(
    ...
    auto has_concrete_write = [&](const concrete_memory_access &access) {
        for (uint64_t offset = 0; offset < access.size; ++offset) {
            const uint64_t byte_offset = access.address.offset + offset;
            if (byte_offset < access.address.offset)
                break;
            if (concretely_written_bytes.count(
                    memory_address{.page = access.address.page, .offset = byte_offset}))
            {
                return true;
            }
    ...
    std::unordered_map<concrete_location_key, uint32_t, concrete_location_hash>
        concrete_location_ids;
    llvm::DenseMap<uint64_t, llvm::MemoryAccess *> concrete_clobbers;

    auto get_concrete_clobber = [&](llvm::MemoryAccess *start,
                                    const concrete_memory_access &load_access) {
        concrete_location_key location{.address = load_access.address, .size = load_access.size};
        auto [location_it, inserted] = concrete_location_ids.emplace(
            location, static_cast<uint32_t>(concrete_location_ids.size()));
        const uint64_t location_id = location_it->second;

        auto make_key = [&](llvm::MemoryAccess *access) {
            uint64_t access_id;
            if (auto *definition = llvm::dyn_cast<llvm::MemoryDef>(access)) {
        ...
        while (cursor) {
            ...
            if (MSSA->isLiveOnEntryDef(cursor) || llvm::isa<llvm::MemoryPhi>(cursor)) {
                clobber = cursor;
                break;
            }
            ...
            auto *store =
                definition ? llvm::dyn_cast<llvm::StoreInst>(definition->getMemoryInst()) : nullptr;
            if (!store) {
                clobber = cursor;
                break;
            }

            const concrete_memory_access *store_access = get_memory_access(store);
            if (!store_access) {
                clobber = cursor;
                break;
            }
            ...
            if (store_access->address.page == load_access.address.page) {
                for (uint64_t offset = 0; offset < load_access.size; ++offset) {
                    ...
                    if (byte_address < store_access->address.offset)
                        continue;
                    if (byte_address - store_access->address.offset < store_access->size) {
                        overlaps = true;
                        break;
                    }
                }
        ...
        if (LI.getLoopFor(block) || llvm::isa<llvm::PHINode>(instruction)) {
            ...
            if (blocks.at(block).predecessors.size() > 1) {
                ...
                if (safe) {
                    ...
                    if (!has_unknown_writes && !has_concrete_write(load_access)
                        && !crosses_any_memory_phi(raw_access))
                    {
                        clobber = MSSA->getLiveOnEntryDef();
                    }
                    else if (!has_unknown_writes) {
                        clobber = get_concrete_clobber(raw_access, load_access);
                    }
                    else {
                        clobber = memory_walker->getClobberingMemoryAccess(load, BatchAA);
                    }
                    ...
                    while (safe && resolved_mask != complete_mask) {
                        ...
                        if (store_mask) {
                            if (LI.getLoopFor(store->getParent())
                                || blocks.at(store->getParent()).predecessors.size() > 1
                        ...
                        if (!has_unknown_writes) {
                            next_clobber = get_concrete_clobber(next_raw, load_access);
                        }
                        else {
                            next_clobber = memory_walker->getClobberingMemoryAccess(
