...
bool vexa::passes::state2alloca::run(llvm::Function *func)
{
    ...
    llvm::AllocaInst *slot = builder->CreateAlloca(builder->getInt32Ty());
    {
        ...
        builder->CreateStore(builder->CreateLoad(builder->getInt32Ty(), addr), slot);
    }
    ...
    auto create_alloca = [&](const remill::Register *r) {
        if (!r->parent) {
            llvm::Value *alloca = builder->CreateAlloca(
                builder->getIntNTy(r->size * 8), builder->getIntN(64, 1), r->name);
    ...
    auto init_alloca = [&](const remill::Register *r) {
        if (!r->parent) {
            auto reg_addr = builder->CreateInBoundsGEP(
                builder->getInt8Ty(), cpu->state_ptr.l, builder->getIntN(64, r->offset), r->name);
    ...
    for (auto &BB : llvm::make_early_inc_range(*func)) {
        ...
        for (auto &I : llvm::make_early_inc_range(BB)) {
            ...
            llvm::Value *offset_gep = builder->CreateInBoundsGEP(
                builder->getInt8Ty(), alloca_ptr, builder->getInt64(offset - parent_r->offset));
    ...
    for (auto &BB : *func) {
        ...
        if (llvm::isa<llvm::ReturnInst>(term)) {
            ...
            for (auto &[offset, alloca] : offset_to_alloca) {
                ...
                auto *reg_addr = builder->CreateInBoundsGEP(
                    builder->getInt8Ty(), cpu->state_ptr.l, builder->getInt64(offset));
