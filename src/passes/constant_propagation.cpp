...
bool vexa::passes::constant_propagation::run()
{
    llvm::Function* function = builder->get_function();
    ...
    {
        // if block is marked by unrolled loop marker, skip
        // so we dont break the loop reroll pass later
        if (false && cpu->p_manager.uloop_backedges.count(&BB))
            continue;
        ...
        for (auto &I : BB)
        {
            ...
            llvm::Value *C = builder->get_const_int(val.as_uint64(), val.get_sort().bv_size()).as_llvm();
    ...
    // taint the load variables that access to tainted addresses that
    // detected by overconcretize memory analysis up there
    for (auto &BB : *function)
        for (auto &I : BB)
    ...
    for (llvm::BasicBlock &BB : *function)
    {
        // if block is marked by unrolled loop marker, skip
        // so we dont break the loop reroll pass later
        if (false && cpu->p_manager.uloop_backedges.count(&BB)) // disabled
            continue;
        ...
        for (auto &I : BB)
        {
            ...
            llvm::Value *concretized = builder->get_const_int(expression.as_uint64(), expression.get_sort().bv_size()).as_llvm();
