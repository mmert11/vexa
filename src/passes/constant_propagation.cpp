...
bool vexa::passes::constant_propagation::has_only_defined_constant_operands(
    llvm::Instruction *instruction)
{
    for (llvm::Use &operand : instruction->operands()) {
        llvm::Value *value = operand.get();
        if (!llvm::isa<llvm::Constant>(value) || llvm::isa<llvm::UndefValue>(value)
...
{
    if (!instruction->getParent() || llvm::isa<llvm::LoadInst>(instruction)
        || llvm::isa<llvm::StoreInst>(instruction) || llvm::isa<llvm::PHINode>(instruction)
        || instruction->isTerminator() || instruction->mayHaveSideEffects()
        || !instruction->getType()->isIntegerTy() || LI.getLoopFor(instruction->getParent())
    ...
    return operation && !llvm::canCreateUndefOrPoison(operation)
           && llvm::isSafeToSpeculativelyExecute(instruction) && symex->is_sync(instruction);
}
...
std::vector<llvm::Instruction *> vexa::passes::constant_propagation::propagate_ssa_values(
    ...
    const std::vector<llvm::Instruction *> &seeds)
{
    ...
    while (!worklist.empty()) {
        ...
        if (folded.count(instruction) || !is_ssa_fold_candidate(instruction, LI, joined_blocks)) {
            continue;
        }
        ...
        if (!value->is_concrete() || value->size() != instruction->getType()->getIntegerBitWidth())
        {
            continue;
        }

        llvm::Constant *constant = builder->getIntN(value->size(), value->as_uint64());
        std::vector<llvm::Instruction *> users = collect_instruction_users(instruction);
...
bool vexa::passes::constant_propagation::run(llvm::Function *function)
{
    ...
    LOG_INFO(logger, "Concretized memory loads -> {}", memory.dead_loads.size());
    LOG_INFO(logger, "Concretized SSA values -> {}", dead_ssa_values.size());
