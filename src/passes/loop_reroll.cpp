...
// it searches for vexa.unrolled.marker function calls in code to identify locations loop starts
...
bool vexa::passes::loop_reroll::run()
{
    ...
    for (auto &BB : *builder->get_function())
    {
        for (auto it = BB.rbegin(), end = BB.rend(); it != end; ++it)
        {
            ...
            for (auto it_ = std::next(it); it_ != end;)
            {
                ...
                if (search_for.size() > 128) {
                    //llvm::errs() << "[LIMIT] Pattern size reached 128, aborting the search.\n";
                    break;
                }
            ...
            if (loop.iteration_count > 3)
            {
                ...
                //                         Used to fix up post-loop uses (e.g. ret) before erasing.
                ...
                // we have re-rolled a loop, now repeat the process until there is nothing to optimize
                return true;
            }
        }
    }
