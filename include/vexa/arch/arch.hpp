...
#define VEXA_VAL(llvm_ptr)                                                                         \
    vexa::dual_value                                                                               \
    {                                                                                              \
        llvm_ptr, symex->get(llvm_ptr)                                                             \
    }
...
class cpu
{
  public:
      ...
      public:
        ...
        emulator(vexa::cpu *_cpu)
            : context(_cpu->context),
              cpu(_cpu),
              builder(_cpu->builder),
              symex(context->symex),
              memory(context->memory),
              DL(context->llvm_module->getDataLayout())
        {}
    ...
    {
        resolved_path_t(uint64_t _true_ip, uint64_t _false_ip)
            : true_ip(_true_ip), false_ip(_false_ip)
        {}
    ...
    struct snapshot
    {
        ...
        std::vector<bw::Term> path_constraints;
    };
